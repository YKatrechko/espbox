...
const char KEY_MENU_SPIFFS [] PROGMEM = "$MENU_SPIFFS$";
const char KEY_MENU_SDFS [] PROGMEM = "$MENU_SDFS$";
...
const char VALUE_SPIFFS [] PROGMEM = "SPIFFS";
const char VALUE_SDFS [] PROGMEM = "SD FS";
...
void handle_web_spiffs()
{
  ...
  if (outputjson) {
    web_interface->generateJSON(KeysList , ValuesList);
  } else { //process the template file and provide list of variables
    //page title and filenames
    web_interface->SetPageProp(KeysList, ValuesList, FPSTR(VALUE_SPIFFS), F("spiffs"));
    //menu item
    KeysList.add(FPSTR(KEY_MENU_SPIFFS));
    ValuesList.add(FPSTR(VALUE_ACTIVE));
    web_interface->processTemplate("/spiffs.tpl", KeysList , ValuesList);
  }
  //need to clean to speed up memory recovery
  KeysList.clear();
  ValuesList.clear();
...
void handle_web_sdfs()
{
  ...
  if (outputjson) {
    ...
    //page title and filenames
    web_interface->SetPageProp(KeysList, ValuesList, FPSTR(VALUE_SDFS), F("sdfs"));
    //menu item
    KeysList.add(FPSTR(KEY_MENU_SDFS));
    ValuesList.add(FPSTR(VALUE_ACTIVE));
    web_interface->processTemplate("/sdfs.tpl", KeysList , ValuesList);
  }
  //need to clean to speed up memory recovery
  KeysList.clear();
  ValuesList.clear();
...
void handleSDFileList()
{
  ...
  if (web_interface->WebServer.hasArg("action")) {
    ...
    if (web_interface->WebServer.arg("action") == "delete" && web_interface->WebServer.hasArg("filename")) {
      ...
      LOG(filename)
      LOG("\r\n")
      sstatus = SDCARD::rmFile(filename) ? "Ok" : "Fail";
      LOG(sstatus)
      LOG("\r\n----\r\n")
    }
    //delete a directory
    if (web_interface->WebServer.arg("action") == "deletedir" && web_interface->WebServer.hasArg("filename")) {
      ...
      LOG(filename)
      LOG("\r\n")
      sstatus = SDCARD::rmDirectory(filename) ? "Ok" : "Fail";
      LOG(sstatus)
      LOG("\r\n----\r\n")
    }
    //create a directory
    if (web_interface->WebServer.arg("action") == "createdir" && web_interface->WebServer.hasArg("filename")) {
      ...
      LOG(filename)
      LOG("\r\n")
      sstatus = SDCARD::mkDirectory(filename) ? "Ok" : "Fail";
      LOG(sstatus)
      LOG("\r\n----\r\n")
    }
  }

  String jsonfile =  SDCARD::jsonDirectory(path, sstatus);
  path = String();
  web_interface->WebServer.sendHeader("Cache-Control", "no-cache");
  web_interface->WebServer.send(200, "application/json", jsonfile);
  web_interface->blockserial = false;
...
#ifdef SDCARD_FEATURE
  File sdfile;
  if (SDCARD::readFile(path, sdfile)) {
    web_interface->WebServer.streamFile(sdfile, contentType);
    sdfile.close();
  }
  else
#endif
